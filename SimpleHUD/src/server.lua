...
if config.enablePriorityStatus then
    local priority = "~s~Peacetime: ~g~Disabled"
    ...
    function priorityCooldown(time)
        ...
        for cooldown = time, 1, -1 do
            if cooldown > 1 then
                priority = "~s~Peacetime Cooldown: ~c~" .. cooldown .. " minutes"
            else
                priority = "~s~Peacetime Cooldown: ~c~" .. cooldown .. " minute"
            end
            TriggerClientEvent("AndyHUD:returnPriority", -1, priority)
            Citizen.Wait(60000)
        end
        priority = "~s~Peacetime: ~g~Disabled"
        TriggerClientEvent("AndyHUD:returnPriority", -1, priority)
    ...
    RegisterCommand("prio-start", function(source, args, rawCommand)
        ...
        priority = "~s~Peacetime: ~r~Active ~c~(" .. tableConcat(priorityPlayers, ", ") .. ")"
        TriggerClientEvent("AndyHUD:returnPriority", -1, priority)
    end, false)
